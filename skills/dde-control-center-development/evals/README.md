...
`trigger-evals.json` 包含 21 条触发测试（13 正例 + 8 反例），其中 5 条为跨 skill 测试（同时涉及 DConfig、DPalette、DCI 图标等 DTK 关键词，验证 `$dtk-development` 交叉触发）。

## 总计

共 **22** 个 output-quality evals（含 eval-template.md），覆盖控制中心插件开发的核心场景。
