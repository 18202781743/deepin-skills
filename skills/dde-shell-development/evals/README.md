...
| 编号 | 文件 | 测试点 |
|------|------|--------|
| eval-001 | dtk-dependency | 使用 DTK DConfig 的跨 skill 依赖加载 |

## 触发评估

`trigger-evals.json` 包含 42 条触发测试（23 正例 + 19 反例），其中 1 条为跨 skill 测试（同时涉及 DConfig、DPalette、DBlurEffectWidget、D.DTK 等 DTK 关键词，验证 `$dtk-development` 交叉触发）。

## 总计

共 **21** 个 output-quality evals（含 eval-template.md），覆盖 dde-shell 插件开发的核心场景。
